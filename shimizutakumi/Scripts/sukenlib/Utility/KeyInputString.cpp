...
bool suken::CKeyInput::CheckAnyActive() {
	return fActiveElse;
}
...
suken::CKeyInputNumF::CKeyInputNumF() {
	str = "";
	link = nullptr;
	size = 9;
	width = 0;
...
suken::CKeyInputNumF::CKeyInputNumF(double* link, char size, int height, const char* font) {
	str = std::to_string((long double)*link);
	this->link = link;
	this->size = size > 9 ? 9 : size;
	this->height = height;
	this->font = CreateFontToHandle(font, height, -1);
...
void suken::CKeyInput::Draw(int x, int y, bool activeOnly) {
	if (!activeOnly) {
		if (!fActiveElse) {
			if (str.size() == 0) {
				if (Input.GetMouseEnter(Input.mouse.L,x, y, x + height, y + height)) {
					fActive = true;
					fActiveElse = true;
					isEscaped = false;
				}
				...
				if (Input.GetMouseEnter(Input.mouse.L, x, y, x + width, y + height)) {
					fActive = true;
					fActiveElse = true;
					isEscaped = false;
				}
...
bool suken::CKeyInput::IsEscaped() {
	return isEscaped;
}
...
void suken::CKeyInputNumF::SetString() {
	str = std::to_string((long double)*link);
	num = str.size();
	width = GetDrawStringWidthToHandle(str.c_str(), str.size(), this->font) + 2;
}
...
void suken::CKeyInputNumF::KeyInput() {
	Key(RIGHT) {
		if (num < str.size()) {
			num++;
		}
