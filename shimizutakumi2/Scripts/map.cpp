...
void Map::Draw(int scrX, int scrY) {//和差積算と比べると除算は超重いのでこのほうがいい
	for (int x = max(0, -scrX / chipSize), endX = min(width - 1, x + System.GetWindowX() / chipSize + 1); x < endX; ++x) {
		for (int y = max(0, -scrY / chipSize), endY = min(height - 1, y + System.GetWindowY() / chipSize + 1); y < endY; ++y) {
...
bool Map::IsWall(int x, int y)const {//仮　ある値[以上/以下]で判定すると軽い
	if (x < 0 || y < 0 || x >= width || y >= height) {//チップ外は壁
		return true;
	}
	return chipData[Code(x, y)] == 2;
}
